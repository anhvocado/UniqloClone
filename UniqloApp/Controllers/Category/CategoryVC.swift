...
class CategoryVC: UIViewController {
    ...
    func getCategoriesAndBrands() {
        CategoryListAPI().execute(success: { [weak self] response in
    ...
    func setupSection() {
        for (index, element) in sections.enumerated() {
            self.sectionButton[index].setTitle(element.name, for: .normal)
        }
    }
...
extension CategoryVC: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.setupData(brands: self.sections[indexPath.item].brands)
